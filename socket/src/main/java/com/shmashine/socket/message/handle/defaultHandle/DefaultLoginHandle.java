...
import com.shmashine.socket.message.bean.ParamPreCfgMessage;
...
public class DefaultLoginHandle implements LoginHandle {
    ...
    private final ExecutorService executorService = new ShmashineThreadPoolExecutor(8, 64,
            ...
            PersistentRejectedExecutionHandler.of("executorService"), "DefaultLoginHandle");
    ...
    @Autowired
    public DefaultLoginHandle(ElevatorStatusHandle elevatorStatusHandle, TblDeviceService deviceService,
    ...
    public boolean loginHandle(MessageData messageData, Channel channel) {
        ...
        if (result) {
            try {
                ...
                //设备传感器配置下发
                executorService.submit(() -> paramPreCfgPush(elevatorCode, sensorType));
    ...
    public void deviceInfoHandle(MessageData messageData, Channel channel) {
        ...
        try {
            ...
            log.error("{}---更新设备信息失败,sensorType:{},elevatorCode:{},messageData:{},error:{}", TimeUtils.nowTime(),
                    sensorType, elevatorCode, messageData, e.getMessage());
        }
    }
     ...
     */
    private void paramPreCfgPush(String elevatorCode, String sensorType) {

        //获取设备参数配置信息
        DeviceParamConfDO deviceParamConf = deviceService.getDeviceParamConf(elevatorCode, sensorType);

        //配置已下发未确认成功状态
        if (deviceParamConf != null && deviceParamConf.getDeviceConfStatus() == 1) {
            //获取设备信息
            TblDevice device = deviceService.getDevice(elevatorCode, sensorType);

            //构建下发消息内容
            ParamPreCfgMessage paramCfgMessage = ParamPreCfgMessage.builder()
                    .etype(device.geteType()).eid(elevatorCode).doorNum(deviceParamConf.getDoorNum())
                    .mode(deviceParamConf.getMode()).sensorMap(deviceParamConf.getSensorCfg())
                    .baseFloor(deviceParamConf.getBaseFloor()).trappedRptTm(deviceParamConf.getPeopleRptTm())
                    .statData(deviceParamConf.getStatData()).overSpeed(deviceParamConf.getOverSpeed())
                    .sensorInvert(deviceParamConf.getSensorInvertCfg()).humanType(deviceParamConf.getHumanType())
                    .ipType(deviceParamConf.getDevIpType()).IP(deviceParamConf.getDevIp())
                    .netmask(deviceParamConf.getDevNetmask()).gateway(deviceParamConf.getDevGateway())
                    .build();

            String floor = deviceParamConf.getFloor();
            if (StringUtils.hasText(floor)) {
                String[] split = floor.split("~");
                paramCfgMessage.setFloorMin(split[0]);
                paramCfgMessage.setFloorMax(split[1]);
            }

            String message = JSON.toJSONString(paramCfgMessage);

            log.info("设备参数配置下发，message：{}", message);
    ...
    private String addSensorIpsToMessage(String elevatorCode, String message) {
        try {
            ...
            messageJson.put(SocketConstants.SENSOR_TYPE_CAR_ROOF,
                    redisService.getDeviceSensorIp(elevatorCode, SocketConstants.SENSOR_TYPE_CAR_ROOF));

            messageJson.put(SocketConstants.SENSOR_TYPE_MOTOR_ROOM,
                    redisService.getDeviceSensorIp(elevatorCode, SocketConstants.SENSOR_TYPE_MOTOR_ROOM));
