...
@Slf4j
@Component
public class FaultHandleV200 implements FaultHandle {
    ...
    @Autowired
    public FaultHandleV200(TblFaultService faultService, TblFaultDefinitionServiceI faultDefinitionService,
    ...
    public void process(String faultId, JSONObject messageJson, String elevatorCode, String sensorType,
        ...
        if (MessageConstants.STYPE_ADD.equals(stype)) {
            ...
            if (shielded == 2) {
                faultMessageLogger.info("{} -- Fault has been shielded ! elevatorCode[{}]  faultType[{}]",
    ...
    public void sensorProcess(JSONObject messageJson, String elevatorCode, String sensorType,
        ...
        //故障解码（目前支持16位故障）
        int fault = Integer.parseInt(faultType);
        ...
        for (int i = 0; i < 16; i++) {
            ...
            if ((1 & fault >> i) == 1) {
                ...
                //传感器故障是否屏蔽
                if (SensorFaultShieldCache.checkSensorFaultShield(elevatorCode, faultCode)) {
                    faultMessageLogger.info("-------传感器故障已屏蔽,不做处理------elevator{}, faultCode:{}",
                            elevatorCode, faultCode);

                } else {
                    sensorFaultHandle(messageJson, elevatorCode, sensorType, String.valueOf(faultCode), stype);
                    faultMessageLogger.info("--------------SensorFault------faultCode:{}", faultCode);
                }
    ...
    public void updateFaultHandle(String faultType, String faultSecondType, String elevatorCode, String sensorType) {
        try {
            ...
            faultMessageLogger.info("updateFaultHandle exception: [{}]", ExceptionUtils.getStackTrace(e));
        }
    }
     ...
     */
    public int checkShielded(String elevatorCode, String faultType, String stype,
                             String faultId, Integer elevatorType) {
    ...
    public void cleanAllFault(String elevatorCode, String sensorType) {
        try {
            ...
            log.info("cleanAllFault exception: [{}]", ExceptionUtils.getStackTrace(e));
        }
    }
