...
public class MotorRoomNettyServerHandle extends SimpleChannelInboundHandler<String> {
    ...
    @Autowired
    public MotorRoomNettyServerHandle(MessageHandle messageHandle, ElevatorStatusHandle elevatorStatusHandle) {
    ...
    public void handlerAdded(ChannelHandlerContext context) {
        ...
        channel.writeAndFlush(SocketConstants.MOTOR_ROOM_NEED_LOGIN);
    }
    ...
    protected void channelRead0(ChannelHandlerContext context, String message) {
        Channel channel = context.channel();
        // 消息处理
        boolean isLogin = messageHandle.messageHandle(message, SocketConstants.SENSOR_TYPE_MOTOR_ROOM, channel);
        if (!isLogin) {
            channel.writeAndFlush(SocketConstants.MOTOR_ROOM_NEED_LOGIN);
        }
    }
