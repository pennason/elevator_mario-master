...
public class SingleBoxNettyServerHandle extends SimpleChannelInboundHandler<String> {
    ...
    @Autowired
    public SingleBoxNettyServerHandle(MessageHandle messageHandle, ElevatorStatusHandle elevatorStatusHandle) {
    ...
    public void handlerAdded(ChannelHandlerContext context) {
        ...
        channel.writeAndFlush(SocketConstants.SINGLEBOX_NEED_LOGIN);
    }
    ...
    protected void channelRead0(ChannelHandlerContext context, String message) {
        Channel channel = context.channel();
        // 消息处理
        boolean isLogin = messageHandle.messageHandle(message, SocketConstants.SENSOR_TYPE_SINGLEBOX, channel);
        if (!isLogin) {
    ...
    public void exceptionCaught(ChannelHandlerContext context, Throwable throwable) {
        ...
        log.error("设备连接异常，e：{}", throwable.getMessage());
    }
}
