...
public class EscalatorNettyServerHandle extends SimpleChannelInboundHandler<String> {
    ...
    @Autowired
    public EscalatorNettyServerHandle(MessageHandle messageHandle, ElevatorStatusHandle elevatorStatusHandle) {
    ...
    public void handlerAdded(ChannelHandlerContext context) {
        ...
        channel.writeAndFlush(SocketConstants.ESCALATOR_NEED_LOGIN);
    }
    ...
    protected void channelRead0(ChannelHandlerContext context, String message) {
        Channel channel = context.channel();
        // 消息处理
        boolean isLogin = messageHandle.messageHandle(message, SocketConstants.SENSOR_TYPE_ESCALATOR, channel);
        if (!isLogin) {
