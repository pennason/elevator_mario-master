package com.shmashine.gateway.redis.utils;
...
public class RedisUtils {
     ...
     * @param time  时间(秒)
     */
    public boolean putList(String key, Object value, long time) {
        try {
            redisTemplate.opsForList().rightPush(key, value);
            if (time > 0) {
                expire(key, time);
     ...
     * @param value 值
     */
    public void putListAll(String key, List<Object> value) {
        try {
     ...
     * @param time  时间(秒)
     */
    public void putListAll(String key, List<Object> value, long time) {
        try {
            redisTemplate.opsForList().rightPushAll(key, value);
            if (time > 0) {
                expire(key, time);
