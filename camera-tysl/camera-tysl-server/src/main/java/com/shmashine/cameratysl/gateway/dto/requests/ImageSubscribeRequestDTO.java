...
@AllArgsConstructor
@NoArgsConstructor
public class ImageSubscribeRequestDTO implements Serializable {
    /**
     ...
     * 推图回调接⼝
     */
    @Schema(title = "推图回调接⼝", description = "推图回调接⼝", required = true)
    private String callback;
    /**
     * 图⽚传输⽅式（imageUrl或imageObject） 1、imageUrl采⽤url地址传输 2、imageObject采⽤容器id和对象id上传 ⽬前仅⽀持Url地址传输
     */
    @Schema(title = "图⽚传输⽅式",
            description = "图⽚传输⽅式（imageUrl或imageObject） 1、imageUrl采⽤url地址传输 2、imageObject采⽤容器id和对象id上传 ⽬前仅⽀持Url地址传输",
            required = true, example = "imageUrl")
