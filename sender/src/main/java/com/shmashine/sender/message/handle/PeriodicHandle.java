...
public class PeriodicHandle {
    private final BizElevatorService bizElevatorService;

    /**
     * 第三方平台code ： 第三方平台推送处理类
     */
    private static ConcurrentHashMap<String, PeriodicSend> senders = new ConcurrentHashMap<>();

    /**
     * 需要监听故障消息的 推送到第三方平台的处理类
     */
    public static void register(PeriodicSend sender) {
    ...
    public void handle(String message) {
        // 报文落日志
        log.info("send periodic : " + message);
        if (!StringUtils.hasText(message)) {
            return;
        }
        try {
            // step1 : message转jsonObject（json格式校验）
            var messageData = JSON.parseObject(message, PeriodicMessage.class);
    ...
    private void dispatch(PeriodicMessage messageData) {
        ...
        for (String ptCode : ptCodeList) {
            ...
            if (null != sender) {
                try {
                    sender.handlePeriodic(messageData);
                } catch (Exception e) {
                    e.printStackTrace();
                    log.error(e.getMessage());
                }
